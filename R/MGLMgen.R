...
rmn <- function(size, alpha, n){
	## alpha can be a vector or a matrix
	## n is optional 
	## size can be a scalar;or a vector; 
	## seed is optional 
	...
	## devided by the last column of alpha, so that we can 
	## get rid of the identifiability issue when fitting. 
	## This is equivalent to subtract the last column of the parameter 
	## matrix from the parameter matrix.
	...
	rdm <- t( sapply( 1:n, function(i, size, prob) 
			return(rmultinom(n=1, size=size[i], prob=prob[i,])), 
		size,prob) )
...
rgdirm <- function(size, alpha, beta, n){
	...
	for(i in 1:k){
		rdm[,c(i,i+1)] <- rdirm(size=rdm[,i],alpha=cbind(alpha[,i], beta[,i]), n)
	}
